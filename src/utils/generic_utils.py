...
class GenericUtils:
    ...
    def save_asset_to_gcs(self, asset, gcs_bucket_name, gcs_output_path, save_filename_prefix):
        ...
        # for multi-format
            ...
            # Check if save_filename_prefix includes an extension
            if "." not in save_filename_prefix:
                raise ValueError(
                    "save_filename_prefix must include file extension when saving bytes."
                )

            filename = f"{save_filename_prefix}"
            local_path = f"/tmp/{filename}"
